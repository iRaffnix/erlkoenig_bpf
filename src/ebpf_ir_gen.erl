...
compute_struct_layout(Fields) ->
    {Layout, _} = lists:foldl(
        fun({FName, FType}, {Acc, Off}) ->
...
%% Pointer-sized default for nested structs
type_size({named, _}) -> 8;
%% Default to 8 bytes
type_size(_) -> 8.

register_ir_maps([], Gen) ->
    Gen;
...
gen_fn(#fn_decl{params = Params, body = Body}, Gen) ->
    ...
    Gen3 =
        case Params of
            [{PName, _} | _] ->
                Gen2#gen{
                    vars = (Gen2#gen.vars)#{PName => v_ctx},
...
gen_stmts([Stmt | Rest], Gen) ->
    ...
    case has_terminator(Gen2) of
        %% Don't generate dead code after return/exit
        true -> Gen2;
...
gen_for(VarName, From, To, Body, Gen) ->
    ...
    Gen7 = Gen6#gen{
        next_reg = Gen6#gen.next_reg + 1,
        vars = (Gen6#gen.vars)#{VarName => IterReg}
    },
    Gen8 = emit(Gen7, #ir_instr{
        op = sub,
        dst = CountReg,
        args = [ToReg, FromReg],
        type = {scalar, u64}
    ...
    Gen19 = emit(Gen18, #ir_instr{
        op = add,
        dst = IterReg,
        args = [IterReg, 1],
        type = {scalar, u64}
    }),
    Gen20 = emit(Gen19, #ir_instr{
        op = sub,
        dst = CountReg,
        args = [CountReg, 1],
        type = {scalar, u64}
...
gen_match_arms(ExprReg, [{Pat, Body} | Rest], JoinLabel, Gen) ->
    ...
    case Pat of
        {lit_pat, Val} ->
            ...
            Gen5 = add_terminator({cond_br, {cmp, ne, ExprReg, ValReg}, NextLabel, ArmLabel}, Gen4),
...
gen_expr({sizeof_expr, TypeExpr, Loc}, Gen) ->
    Size = resolve_sizeof(TypeExpr, Gen),
    Dst = fresh_reg(Gen),
    Gen2 = Gen#gen{next_reg = Gen#gen.next_reg + 1},
    Gen3 = emit(Gen2, #ir_instr{
        op = mov,
...
gen_map_lookup([MapArg, KeyArg], Gen) ->
    ...
    Gen17 = add_terminator({cond_br, {cmp, eq, CallDst, ZeroReg}, NullLabel, ValueLabel}, Gen16),
...
%% Resolve sizeof: for primitives use type_size/1, for named types look up struct layout.
resolve_sizeof({named, TypeName}, #gen{structs = Structs}) ->
    case maps:find(TypeName, Structs) of
        {ok, Layout} -> struct_total_size(Layout);
        %% Unknown type
        error -> 0
    end;
resolve_sizeof(TypeExpr, _Gen) ->
    type_size(TypeExpr).

%% Compute total size of a struct from its layout.
struct_total_size([]) -> 0;
struct_total_size(Layout) -> lists:max([Offset + Size || {_, Offset, Size} <- Layout]).
...
binop_to_ir('<<') -> lsh;
binop_to_ir('>>') -> rsh;
%% Compare via subtract (cond_br on zero)
binop_to_ir('==') -> sub;
binop_to_ir('!=') -> sub;
binop_to_ir('<') -> sub;
binop_to_ir('>') -> sub;
