%% @doc EBL (Erlkoenig BPF Language) lexer.
%%
...
-type token_type() ::
    %% Literals
    integer_lit | true_kw | false_kw | atom_lit |
    %% Identifiers
    ident | type_ident |
    %% Keywords
    do_kw | end_kw | fn_kw | let_kw | if_kw | elif_kw | else_kw |
    for_kw | in_kw | match_kw | return_kw | type_kw | map_kw |
    const_kw | some_kw | none_kw | sizeof_kw | break_kw | continue_kw |
    %% Program types
    xdp_kw | tc_kw | cgroup_kw | socket_kw |
    %% Primitive types
    u8_kw | u16_kw | u32_kw | u64_kw |
    i8_kw | i16_kw | i32_kw | i64_kw | bool_kw |
    action_kw |
    %% Map types
    hash_kw | array_kw | lru_hash_kw | percpu_hash_kw |
    percpu_array_kw | lru_percpu_hash_kw |
    ringbuf_kw | devmap_hash_kw | prog_array_kw |
    %% Operators
    pipe | pipe_op | or_or | and_and |
    caret | ampersand |
...
scan(<<"?.", Rest/binary>>, Line, Col, Acc) ->
    scan(Rest, Line, Col + 2, [tok(question_dot, '?.', Line, Col) | Acc]);
...
scan(<<$:, C, Rest/binary>>, Line, Col, Acc)
  when (C >= $a andalso C =< $z); C =:= $_ ->
    ...
    scan(Rest2, Line, Col + 1 + Len,
         [#token{type = atom_lit, value = Name, line = Line, col = Col} | Acc]);

%% Single-character operators and delimiters
scan(<<$|, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(pipe_op, '|', L, C) | Acc]);
scan(<<$^, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(caret, '^', L, C) | Acc]);
scan(<<$&, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(ampersand, '&', L, C) | Acc]);
scan(<<$<, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(lt, '<', L, C) | Acc]);
scan(<<$>, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(gt, '>', L, C) | Acc]);
scan(<<$+, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(plus, '+', L, C) | Acc]);
scan(<<$-, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(minus, '-', L, C) | Acc]);
scan(<<$*, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(star, '*', L, C) | Acc]);
scan(<<$/, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(slash, '/', L, C) | Acc]);
scan(<<$%, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(percent, '%', L, C) | Acc]);
scan(<<$!, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(bang, '!', L, C) | Acc]);
scan(<<$~, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(tilde, '~', L, C) | Acc]);
scan(<<$(, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(lparen, '(', L, C) | Acc]);
scan(<<$), Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(rparen, ')', L, C) | Acc]);
scan(<<${, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(lbrace, '{', L, C) | Acc]);
scan(<<$}, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(rbrace, '}', L, C) | Acc]);
scan(<<$[, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(lbracket, '[', L, C) | Acc]);
scan(<<$], Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(rbracket, ']', L, C) | Acc]);
scan(<<$,, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(comma, ',', L, C) | Acc]);
scan(<<$:, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(colon, ':', L, C) | Acc]);
scan(<<$;, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(semicolon, ';', L, C) | Acc]);
scan(<<$., Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(dot, '.', L, C) | Acc]);
scan(<<$=, Rest/binary>>, L, C, Acc) -> scan(Rest, L, C+1, [tok(eq, '=', L, C) | Acc]);

%% Number literals
scan(<<"0x", Rest/binary>>, Line, Col, Acc) ->
    {Val, Rest2, Len} = scan_hex(Rest, 0, 0),
    scan(Rest2, Line, Col + 2 + Len,
...
scan_hex(<<C, Rest/binary>>, Val, Len) when C >= $A, C =< $F ->
    scan_hex(Rest, Val * 16 + (C - $A + 10), Len + 1);
scan_hex(<<$_, C, Rest/binary>>, Val, Len) when (C >= $0 andalso C =< $9);
                                                 (C >= $a andalso C =< $f);
                                                 (C >= $A andalso C =< $F) ->
...
scan_ident_chars(<<C, Rest/binary>>, Acc)
