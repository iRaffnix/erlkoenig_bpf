%% @doc Pure Erlang BPF virtual machine.
%%
...
%% @doc Run a BPF program with pre-existing maps.
...
-spec run_stateful(binary(), map(),
                   {#{non_neg_integer() => ets:tid()},
                    #{non_neg_integer() => #map_meta{}}},
                   map()) ->
    {ok, non_neg_integer(),
     {#{non_neg_integer() => ets:tid()},
      #{non_neg_integer() => #map_meta{}}}} |
...
-spec destroy_maps({#{non_neg_integer() => ets:tid()},
                    #{non_neg_integer() => #map_meta{}}}) -> ok.
...
%% === ALU64 immediate ===
exec_insn(#vm_insn{op = Op, dst = Dst, imm = Imm}, St)
...
%% === ALU64 register ===
exec_insn(#vm_insn{op = Op, dst = Dst, src = Src}, St)
...
%% === Memory LDX ===
exec_insn(#vm_insn{op = Op, dst = Dst, src = Src, off = Off}, St)
...
%% === Memory ST (immediate) ===
exec_insn(#vm_insn{op = Op, dst = Dst, off = Off, imm = Imm}, St)
...
strip_suffixes(S, [Suffix | Rest]) ->
    ...
    case length(S) > SuffLen andalso
         lists:suffix(Suffix, S) of
        true ->
