...
register_maps([], Env) -> Env;
register_maps([#map_decl{name = Name, kind = Kind, key_type = KT,
                          value_type = VT, max_entries = Max} | Rest], Env) ->
    Env2 = Env#env{maps = (Env#env.maps)#{Name => {Kind, ast_type_to_ir(KT),
                                                     ast_type_to_ir(VT), Max}}},
    register_maps(Rest, Env2).

register_consts([], Env) -> Env;
...
register_fns([], Env) -> Env;
register_fns([#fn_decl{name = Name, params = Params, ret_type = RT} | Rest], Env) ->
    ParamTypes = [case PT of undefined -> {scalar, u64}; _ -> ast_type_to_ir(PT) end
                  || {_, PT} <- Params],
    RetType = case RT of undefined -> {scalar, u64}; _ -> ast_type_to_ir(RT) end,
...
check_fns([#fn_decl{params = Params, body = Body} | Rest], Env) ->
    ...
    {_, Locals} = lists:foldl(fun({PName, PT}, {IsFirst, Acc}) ->
        T = case {PT, IsFirst} of
...
check_stmt({if_stmt, Cond, Then, Elifs, Else, _Loc}, Env) ->
    ...
    Env5 = lists:foldl(fun({ECond, EBody}, E) ->
...
check_stmt({for_stmt, VarName, From, To, Body, Loc}, Env) ->
    ...
    Env4 = case is_integer_type(FromT) andalso is_integer_type(ToT) of
        true -> Env3;
...
check_stmt({break_stmt, _Loc}, Env) -> Env;
check_stmt({continue_stmt, _Loc}, Env) -> Env;
            ...
            case maps:find(Name, Env#env.consts) of
                ...
                error ->
                    ...
                    case maps:is_key(Name, Env#env.maps) of
                        true -> {{scalar, u64}, Env};
                        false ->
                            {{scalar, u64}, add_error({undefined_var, Name, Loc}, Env)}
                    end
            end
    end;
...
infer_expr({binop, Op, Left, Right, Loc}, Env) ->
    ...
    case is_comparison_op(Op) of
        true ->
            Env4 = case types_compatible(LT, RT) of
                true -> Env3;
...
infer_expr({struct_lit, TypeName, Fields, Loc}, Env) ->
    case maps:find(TypeName, Env#env.structs) of
        {ok, _StructFields} ->
            Env2 = lists:foldl(fun({_FN, FExpr}, E) ->
...
infer_method(_ObjType, _Obj, _Method, Args, _Loc, Env) ->
    {_ArgTypes, Env2} = infer_exprs(Args, Env),
    {{scalar, u64}, Env2}.
...
infer_field({ptr, ctx}, Field, Loc, Env) ->
    ...
    case ebpf_ctx:field(Env#env.prog_type, Field) of
        ...
        {ok, _Offset, _Size} -> {{scalar, u64}, Env};
        {error, unknown_field} ->
...
types_compatible(T, T) -> true;
types_compatible({scalar, A}, {scalar, B}) ->
    is_integer_type({scalar, A}) andalso is_integer_type({scalar, B});
types_compatible(action, {scalar, _}) -> true;
types_compatible({scalar, _}, action) -> true;
...
wider_scalar(A, B) ->
    ...
    case {is_unsigned(A), is_unsigned(B)} of
        ...
        _Mixed         -> unsigned_of_width(Width)  %% mixed → unsigned
    end.

scalar_width(u8)  -> 8;  scalar_width(i8)  -> 8;
scalar_width(u16) -> 16; scalar_width(i16) -> 16;
scalar_width(u32) -> 32; scalar_width(i32) -> 32;
scalar_width(u64) -> 64; scalar_width(i64) -> 64;
scalar_width(bool) -> 1;
scalar_width(_)   -> 64.

is_unsigned(u8)  -> true; is_unsigned(u16) -> true;
is_unsigned(u32) -> true; is_unsigned(u64) -> true;
...
signed_of_width(1)  -> i8;   %% bool promoted to signed → i8
...
expect_bool({scalar, bool}, _Expr, Env) -> Env;
expect_bool({scalar, T}, _Expr, Env) when
      T =:= u8; T =:= u16; T =:= u32; T =:= u64;
      T =:= i8; T =:= i16; T =:= i32; T =:= i64 -> Env;
