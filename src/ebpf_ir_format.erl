...
format_instr(#ir_instr{op = Op, dst = Dst, args = [A, B]}) when
    Op =:= add;
    Op =:= sub;
    Op =:= mul;
    Op =:= 'div';
...
format_label({label, N}) -> iolist_to_binary(io_lib:format("L~B", [N])).
