...
-moduledoc """
Test packet generator for XDP testing.
...
tcp(Opts) ->
    ...
    TcpHdr =
        <<SrcPort:16/big, DstPort:16/big,
            % seq
            0:32,
            % ack
            0:32, 5:4, 0:3, FlagBits:9/big,
            % window
            65535:16/big,
...
arp(Opts) ->
    ...
    ArpPayload =
        <<1:16/big,
            ...
            4:8,
            % operation: request
            1:16/big, SenderMac:6/binary, SenderIPInt:32/big, TargetMac:6/binary,
            TargetIPInt:32/big>>,
...
-doc "Build an XDP context map for the Erlang VM from a packet binary. Returns #{ctx => CtxBin, packet => PktBin} suitable for ebpf_vm:run/2.".
...
ipv4_header(Protocol, Payload, SrcIP, DstIP, TTL) ->
    ...
    HdrNoChk =
        <<4:4, 5:4, 0:8, TotalLen:16/big, 0:16, 16#4000:16/big, TTL:8, Protocol:8, 0:16,
            SrcIPInt:32/big, DstIPInt:32/big>>,
    Chk = ip_checksum(HdrNoChk),
    <<4:4, 5:4, 0:8, TotalLen:16/big, 0:16, 16#4000:16/big, TTL:8, Protocol:8, Chk:16/big,
        SrcIPInt:32/big, DstIPInt:32/big>>.
