...
%% ALU64 immediate
format_insn(#vm_insn{op = Op, dst = Dst, imm = Imm})
  when Op =:= add64_imm; Op =:= sub64_imm; Op =:= mul64_imm; Op =:= div64_imm;
       Op =:= or64_imm; Op =:= and64_imm; Op =:= lsh64_imm; Op =:= rsh64_imm;
       Op =:= mod64_imm; Op =:= xor64_imm; Op =:= mov64_imm; Op =:= arsh64_imm ->
    Name = alu_name(Op),
    iolist_to_binary(io_lib:format("~s r~B, ~s", [Name, Dst, fmt_imm(Imm)]));

%% ALU64 register
format_insn(#vm_insn{op = Op, dst = Dst, src = Src})
  when Op =:= add64_reg; Op =:= sub64_reg; Op =:= mul64_reg; Op =:= div64_reg;
       Op =:= or64_reg; Op =:= and64_reg; Op =:= lsh64_reg; Op =:= rsh64_reg;
       Op =:= mod64_reg; Op =:= xor64_reg; Op =:= mov64_reg; Op =:= arsh64_reg ->
    Name = alu_name(Op),
...
%% ALU32 immediate
format_insn(#vm_insn{op = Op, dst = Dst, imm = Imm})
  when Op =:= add32_imm; Op =:= sub32_imm; Op =:= mul32_imm; Op =:= div32_imm;
       Op =:= or32_imm; Op =:= and32_imm; Op =:= lsh32_imm; Op =:= rsh32_imm;
       Op =:= mod32_imm; Op =:= xor32_imm; Op =:= mov32_imm; Op =:= arsh32_imm ->
    Name = alu_name(Op),
    iolist_to_binary(io_lib:format("~s r~B, ~s", [Name, Dst, fmt_imm(Imm)]));

%% ALU32 register
format_insn(#vm_insn{op = Op, dst = Dst, src = Src})
  when Op =:= add32_reg; Op =:= sub32_reg; Op =:= mul32_reg; Op =:= div32_reg;
       Op =:= or32_reg; Op =:= and32_reg; Op =:= lsh32_reg; Op =:= rsh32_reg;
       Op =:= mod32_reg; Op =:= xor32_reg; Op =:= mov32_reg; Op =:= arsh32_reg ->
    Name = alu_name(Op),
    iolist_to_binary(io_lib:format("~s r~B, r~B", [Name, Dst, Src]));
...
format_insn(#vm_insn{op = Op, dst = Dst, off = Off, imm = Imm})
  when Op =:= jeq_imm; Op =:= jgt_imm; Op =:= jge_imm; Op =:= jset_imm;
       Op =:= jne_imm; Op =:= jsgt_imm; Op =:= jsge_imm;
       Op =:= jlt_imm; Op =:= jle_imm; Op =:= jslt_imm; Op =:= jsle_imm ->
    Name = jmp_name(Op),
    iolist_to_binary(io_lib:format("~s r~B, ~s, ~s", [Name, Dst, fmt_imm(Imm), fmt_jmp_off(Off)]));

%% JMP64 conditional (register)
format_insn(#vm_insn{op = Op, dst = Dst, src = Src, off = Off})
  when Op =:= jeq_reg; Op =:= jgt_reg; Op =:= jge_reg; Op =:= jset_reg;
       Op =:= jne_reg; Op =:= jsgt_reg; Op =:= jsge_reg;
       Op =:= jlt_reg; Op =:= jle_reg; Op =:= jslt_reg; Op =:= jsle_reg ->
    Name = jmp_name(Op),
    iolist_to_binary(io_lib:format("~s r~B, r~B, ~s", [Name, Dst, Src, fmt_jmp_off(Off)]));

%% JMP32 conditional (immediate)
format_insn(#vm_insn{op = Op, dst = Dst, off = Off, imm = Imm})
  when Op =:= jeq32_imm; Op =:= jgt32_imm; Op =:= jge32_imm; Op =:= jset32_imm;
       Op =:= jne32_imm; Op =:= jsgt32_imm; Op =:= jsge32_imm;
       Op =:= jlt32_imm; Op =:= jle32_imm; Op =:= jslt32_imm; Op =:= jsle32_imm ->
    Name = jmp_name(Op),
    iolist_to_binary(io_lib:format("~s r~B, ~s, ~s", [Name, Dst, fmt_imm(Imm), fmt_jmp_off(Off)]));

%% JMP32 conditional (register)
format_insn(#vm_insn{op = Op, dst = Dst, src = Src, off = Off})
  when Op =:= jeq32_reg; Op =:= jgt32_reg; Op =:= jge32_reg; Op =:= jset32_reg;
       Op =:= jne32_reg; Op =:= jsgt32_reg; Op =:= jsge32_reg;
       Op =:= jlt32_reg; Op =:= jle32_reg; Op =:= jslt32_reg; Op =:= jsle32_reg ->
    Name = jmp_name(Op),
    iolist_to_binary(io_lib:format("~s r~B, r~B, ~s", [Name, Dst, Src, fmt_jmp_off(Off)]));
...
alu_name(add64_imm) -> "add64"; alu_name(add64_reg) -> "add64";
alu_name(sub64_imm) -> "sub64"; alu_name(sub64_reg) -> "sub64";
alu_name(mul64_imm) -> "mul64"; alu_name(mul64_reg) -> "mul64";
alu_name(div64_imm) -> "div64"; alu_name(div64_reg) -> "div64";
alu_name(or64_imm)  -> "or64";  alu_name(or64_reg)  -> "or64";
alu_name(and64_imm) -> "and64"; alu_name(and64_reg) -> "and64";
alu_name(lsh64_imm) -> "lsh64"; alu_name(lsh64_reg) -> "lsh64";
alu_name(rsh64_imm) -> "rsh64"; alu_name(rsh64_reg) -> "rsh64";
alu_name(mod64_imm) -> "mod64"; alu_name(mod64_reg) -> "mod64";
alu_name(xor64_imm) -> "xor64"; alu_name(xor64_reg) -> "xor64";
alu_name(mov64_imm) -> "mov64"; alu_name(mov64_reg) -> "mov64";
alu_name(arsh64_imm) -> "arsh64"; alu_name(arsh64_reg) -> "arsh64";
alu_name(add32_imm) -> "add32"; alu_name(add32_reg) -> "add32";
alu_name(sub32_imm) -> "sub32"; alu_name(sub32_reg) -> "sub32";
alu_name(mul32_imm) -> "mul32"; alu_name(mul32_reg) -> "mul32";
alu_name(div32_imm) -> "div32"; alu_name(div32_reg) -> "div32";
alu_name(or32_imm)  -> "or32";  alu_name(or32_reg)  -> "or32";
alu_name(and32_imm) -> "and32"; alu_name(and32_reg) -> "and32";
alu_name(lsh32_imm) -> "lsh32"; alu_name(lsh32_reg) -> "lsh32";
alu_name(rsh32_imm) -> "rsh32"; alu_name(rsh32_reg) -> "rsh32";
alu_name(mod32_imm) -> "mod32"; alu_name(mod32_reg) -> "mod32";
alu_name(xor32_imm) -> "xor32"; alu_name(xor32_reg) -> "xor32";
alu_name(mov32_imm) -> "mov32"; alu_name(mov32_reg) -> "mov32";
alu_name(arsh32_imm) -> "arsh32"; alu_name(arsh32_reg) -> "arsh32".

jmp_name(jeq_imm) -> "jeq"; jmp_name(jeq_reg) -> "jeq";
jmp_name(jgt_imm) -> "jgt"; jmp_name(jgt_reg) -> "jgt";
jmp_name(jge_imm) -> "jge"; jmp_name(jge_reg) -> "jge";
jmp_name(jset_imm) -> "jset"; jmp_name(jset_reg) -> "jset";
jmp_name(jne_imm) -> "jne"; jmp_name(jne_reg) -> "jne";
jmp_name(jsgt_imm) -> "jsgt"; jmp_name(jsgt_reg) -> "jsgt";
jmp_name(jsge_imm) -> "jsge"; jmp_name(jsge_reg) -> "jsge";
jmp_name(jlt_imm) -> "jlt"; jmp_name(jlt_reg) -> "jlt";
jmp_name(jle_imm) -> "jle"; jmp_name(jle_reg) -> "jle";
jmp_name(jslt_imm) -> "jslt"; jmp_name(jslt_reg) -> "jslt";
jmp_name(jsle_imm) -> "jsle"; jmp_name(jsle_reg) -> "jsle";
jmp_name(jeq32_imm) -> "jeq32"; jmp_name(jeq32_reg) -> "jeq32";
jmp_name(jgt32_imm) -> "jgt32"; jmp_name(jgt32_reg) -> "jgt32";
jmp_name(jge32_imm) -> "jge32"; jmp_name(jge32_reg) -> "jge32";
jmp_name(jset32_imm) -> "jset32"; jmp_name(jset32_reg) -> "jset32";
jmp_name(jne32_imm) -> "jne32"; jmp_name(jne32_reg) -> "jne32";
jmp_name(jsgt32_imm) -> "jsgt32"; jmp_name(jsgt32_reg) -> "jsgt32";
jmp_name(jsge32_imm) -> "jsge32"; jmp_name(jsge32_reg) -> "jsge32";
jmp_name(jlt32_imm) -> "jlt32"; jmp_name(jlt32_reg) -> "jlt32";
jmp_name(jle32_imm) -> "jle32"; jmp_name(jle32_reg) -> "jle32";
jmp_name(jslt32_imm) -> "jslt32"; jmp_name(jslt32_reg) -> "jslt32";
jmp_name(jsle32_imm) -> "jsle32"; jmp_name(jsle32_reg) -> "jsle32".
...
%% @doc Generate a human-readable explanation of a decoded BPF instruction.
%% Returns a map with 'short' (one-line summary) and 'detail' (full explanation).
-spec explain_insn(#vm_insn{}) -> #{short := binary(), detail := binary(),
                                     category := <<_:32, _:_*8>>}.

explain_insn(#vm_insn{op = nop}) ->
    #{short => <<"No operation">>,
      detail => <<"Second half of a 16-byte LD_IMM64 instruction. "
...
explain_insn(#vm_insn{op = exit_insn}) ->
    #{short => <<"Program exit  - return R0 to caller">>,
      detail => <<"Terminates the BPF program. The value in R0 is returned "
...
explain_insn(#vm_insn{op = mov64_imm, dst = Dst, imm = Imm}) ->
    #{short => fmt("Set r~B to ~s", [Dst, explain_value(Imm)]),
      detail => fmt("Load a 32-bit immediate value into a 64-bit register. "
...
explain_insn(#vm_insn{op = mov64_reg, dst = Dst, src = Src}) ->
    #{short => fmt("Copy r~B into r~B", [Src, Dst]),
      detail => fmt("Register-to-register move (64-bit). Copies the full "
                    "64-bit value. ~s",
...
explain_insn(#vm_insn{op = mov32_imm, dst = Dst, imm = Imm}) ->
    #{short => fmt("Set lower 32 bits of r~B to ~B, zero upper 32", [Dst, Imm]),
      detail => <<"32-bit move clears the upper 32 bits of the destination register. "
...
explain_insn(#vm_insn{op = mov32_reg, dst = Dst, src = Src}) ->
    #{short => fmt("Copy lower 32 bits of r~B into r~B, zero upper 32", [Src, Dst]),
      detail => <<"32-bit register move with automatic zero-extension. "
...
explain_insn(#vm_insn{op = ld64_imm, dst = Dst, imm = Imm}) ->
    #{short => fmt("Load 64-bit immediate ~s into r~B", [explain_value(Imm), Dst]),
      detail => <<"LD_IMM64 is the only BPF instruction that spans 16 bytes "
...
explain_insn(#vm_insn{op = ld_map_fd, dst = Dst, imm = Fd}) ->
    #{short => fmt("Load map descriptor ~B into r~B", [Fd, Dst]),
      detail => fmt("Loads a BPF map file descriptor (fd=~B) into a register. "
                    "In the kernel, this is resolved to a map pointer by the verifier. "
                    "In our VM, the fd is a map index used by helper calls "
                    "(map_lookup_elem, map_update_elem).", [Fd]),
      category => <<"maps">>};

%% ALU64
explain_insn(#vm_insn{op = Op, dst = Dst, imm = Imm})
  when Op =:= add64_imm; Op =:= sub64_imm; Op =:= mul64_imm; Op =:= div64_imm;
       Op =:= mod64_imm; Op =:= and64_imm; Op =:= or64_imm; Op =:= xor64_imm;
       Op =:= lsh64_imm; Op =:= rsh64_imm; Op =:= arsh64_imm ->
    {OpName, OpDesc} = explain_alu(Op),
    #{short => fmt("r~B ~s ~s (64-bit)", [Dst, OpName, explain_value(Imm)]),
      detail => fmt("~s Uses 64-bit arithmetic (full register width). "
                    "Result is stored back in r~B.", [OpDesc, Dst]),
      category => <<"arithmetic">>};
explain_insn(#vm_insn{op = Op, dst = Dst, src = Src})
  when Op =:= add64_reg; Op =:= sub64_reg; Op =:= mul64_reg; Op =:= div64_reg;
       Op =:= mod64_reg; Op =:= and64_reg; Op =:= or64_reg; Op =:= xor64_reg;
       Op =:= lsh64_reg; Op =:= rsh64_reg; Op =:= arsh64_reg ->
    {OpName, OpDesc} = explain_alu(Op),
    #{short => fmt("r~B ~s r~B (64-bit)", [Dst, OpName, Src]),
      detail => fmt("~s Uses 64-bit arithmetic. "
                    "Result is stored back in r~B.", [OpDesc, Dst]),
      category => <<"arithmetic">>};
explain_insn(#vm_insn{op = neg64, dst = Dst}) ->
    #{short => fmt("Negate r~B (two's complement, 64-bit)", [Dst]),
      detail => <<"Two's complement negation: r = -r. Equivalent to 0 - r. "
...
explain_insn(#vm_insn{op = Op, dst = Dst, imm = Imm})
  when Op =:= add32_imm; Op =:= sub32_imm; Op =:= mul32_imm; Op =:= div32_imm;
       Op =:= mod32_imm; Op =:= and32_imm; Op =:= or32_imm; Op =:= xor32_imm;
       Op =:= lsh32_imm; Op =:= rsh32_imm; Op =:= arsh32_imm ->
    {OpName, OpDesc} = explain_alu(Op),
    #{short => fmt("r~B ~s ~s (32-bit, zero-extends)", [Dst, OpName, explain_value(Imm)]),
      detail => fmt("~s 32-bit operation: uses only the lower 32 bits, then "
                    "zero-extends the result to 64 bits (BPF rule).", [OpDesc]),
      category => <<"arithmetic">>};
explain_insn(#vm_insn{op = Op, dst = Dst, src = Src})
  when Op =:= add32_reg; Op =:= sub32_reg; Op =:= mul32_reg; Op =:= div32_reg;
       Op =:= mod32_reg; Op =:= and32_reg; Op =:= or32_reg; Op =:= xor32_reg;
       Op =:= lsh32_reg; Op =:= rsh32_reg; Op =:= arsh32_reg ->
    {OpName, OpDesc} = explain_alu(Op),
...
%% Memory LDX
explain_insn(#vm_insn{op = Op, dst = Dst, src = Src, off = Off})
  when Op =:= ldxb; Op =:= ldxh; Op =:= ldxw; Op =:= ldxdw ->
    {Size, SizeName} = ldx_size(Op),
    Region = explain_memory_region(Src, Off),
    #{short => fmt("Load ~s from memory [r~B~s] into r~B",
                   [SizeName, Src, fmt_off_str(Off), Dst]),
      detail => fmt("Read ~B byte(s) from address (r~B + ~B). ~s "
...
%% Memory STX
explain_insn(#vm_insn{op = Op, dst = Dst, src = Src, off = Off})
  when Op =:= stxb; Op =:= stxh; Op =:= stxw; Op =:= stxdw ->
    {Size, SizeName} = stx_size(Op),
    Region = explain_memory_region(Dst, Off),
    #{short => fmt("Store ~s from r~B into memory [r~B~s]",
                   [SizeName, Src, Dst, fmt_off_str(Off)]),
      detail => fmt("Write ~B byte(s) from register r~B to address (r~B + ~B). ~s "
                    "This is a STORE operation  - register to memory.",
                    [Size, Src, Dst, Off, Region]),
      category => <<"memory">>};

%% Memory ST (immediate)
explain_insn(#vm_insn{op = Op, dst = Dst, off = Off, imm = Imm})
  when Op =:= stb; Op =:= sth; Op =:= stw; Op =:= stdw ->
    {Size, SizeName} = st_size(Op),
    #{short => fmt("Store immediate ~s (~s) to [r~B~s]",
                   [SizeName, explain_value(Imm), Dst, fmt_off_str(Off)]),
      detail => fmt("Write ~B byte(s) of the immediate value ~B to memory at "
                    "(r~B + ~B). Used for initializing stack memory or map keys/values "
                    "with constants.", [Size, Imm, Dst, Off]),
      category => <<"memory">>};
...
explain_insn(#vm_insn{op = ja, off = Off}) ->
    ...
    #{short => fmt("Unconditional jump ~s~B instructions", [sign(Off), abs(Off)]),
      detail => fmt("Jump forward/backward by ~B instruction(s) (to PC+~B). "
...
explain_insn(#vm_insn{op = Op, dst = Dst, off = Off, imm = Imm})
  when Op =:= jeq_imm; Op =:= jgt_imm; Op =:= jge_imm; Op =:= jset_imm;
       Op =:= jne_imm; Op =:= jsgt_imm; Op =:= jsge_imm;
       Op =:= jlt_imm; Op =:= jle_imm; Op =:= jslt_imm; Op =:= jsle_imm;
       Op =:= jeq32_imm; Op =:= jgt32_imm; Op =:= jge32_imm; Op =:= jset32_imm;
       Op =:= jne32_imm; Op =:= jsgt32_imm; Op =:= jsge32_imm;
       Op =:= jlt32_imm; Op =:= jle32_imm; Op =:= jslt32_imm; Op =:= jsle32_imm ->
    {CmpStr, CmpDesc, Signed} = explain_jmp(Op),
    Width = case is_jmp32(Op) of true -> <<"32-bit">>; false -> <<"64-bit">> end,
    #{short => fmt("If r~B ~s ~s, jump ~s~B", [Dst, CmpStr, explain_value(Imm), sign(Off), abs(Off)]),
      detail => fmt("Conditional branch: compare r~B ~s ~B (~s, ~s). "
...
explain_insn(#vm_insn{op = Op, dst = Dst, src = Src, off = Off})
  when Op =:= jeq_reg; Op =:= jgt_reg; Op =:= jge_reg; Op =:= jset_reg;
       Op =:= jne_reg; Op =:= jsgt_reg; Op =:= jsge_reg;
       Op =:= jlt_reg; Op =:= jle_reg; Op =:= jslt_reg; Op =:= jsle_reg;
       Op =:= jeq32_reg; Op =:= jgt32_reg; Op =:= jge32_reg; Op =:= jset32_reg;
       Op =:= jne32_reg; Op =:= jsgt32_reg; Op =:= jsge32_reg;
       Op =:= jlt32_reg; Op =:= jle32_reg; Op =:= jslt32_reg; Op =:= jsle32_reg ->
    {CmpStr, CmpDesc, Signed} = explain_jmp(Op),
    Width = case is_jmp32(Op) of true -> <<"32-bit">>; false -> <<"64-bit">> end,
    #{short => fmt("If r~B ~s r~B, jump ~s~B", [Dst, CmpStr, Src, sign(Off), abs(Off)]),
      detail => fmt("Conditional branch: compare r~B ~s r~B (~s, ~s). "
...
explain_insn(#vm_insn{op = call, imm = Id}) ->
    ...
    #{short => fmt("Call helper: ~s", [Name]),
      detail => fmt("BPF helper call #~B (~s). ~s "
...
explain_memory_region(6, Off) when Off >= 0, Off =< 16 ->
    fmt("R6 typically holds the XDP context pointer (xdp_md). "
        "Offset ~B accesses: ~s.",
        [Off, explain_ctx_field(Off)]);
explain_memory_region(10, Off) when Off < 0 ->
    fmt("R10 is the frame pointer (stack base). "
        "Negative offset ~B = stack slot used for local variables, "
        "map keys, or spilled registers.", [Off]);
...
explain_alu(Op) ->
    ...
    case Base of
        add -> {<<"+">>, <<"Addition: dst = dst + src/imm.">>};
        sub -> {<<"-">>, <<"Subtraction: dst = dst - src/imm.">>};
        mul -> {<<"*">>, <<"Multiplication: dst = dst * src/imm.">>};
        'div' -> {<<"/">>, <<"Unsigned division: dst = dst / src/imm. Division by zero returns 0 in BPF.">>};
        mod -> {<<"%">>, <<"Unsigned modulo: dst = dst %% src/imm.">>};
        and_op -> {<<"&">>, <<"Bitwise AND. Used for masking bits (e.g., extracting TCP flags).">>};
        or_op -> {<<"|">>, <<"Bitwise OR. Used for combining flags.">>};
        xor_op -> {<<"^">>, <<"Bitwise XOR. Used for flipping bits or simple hashing.">>};
        lsh -> {<<"<<">>, <<"Logical left shift: dst = dst << src/imm. Multiplies by powers of 2.">>};
        rsh -> {<<">>">>, <<"Logical right shift (unsigned): fills upper bits with zeros.">>};
        arsh -> {<<">>>">>, <<"Arithmetic right shift (signed): preserves the sign bit.">>};
        mov -> {<<"=">>, <<"Move/assign value.">>};
        _ -> {fmt("~p", [Base]), fmt("ALU operation: ~p.", [Base])}
    end.

strip_alu_suffix(Op) ->
    S = atom_to_list(Op),
    Stripped = lists:foldl(fun(Suffix, Acc) ->
        case lists:suffix(Suffix, Acc) andalso length(Acc) > length(Suffix) of
            true -> lists:sublist(Acc, length(Acc) - length(Suffix));
            false -> Acc
        end
    end, S, ["64_imm", "64_reg", "32_imm", "32_reg"]),
...
explain_jmp_context(Op) ->
    case strip_jmp_suffix(Op) of
        jne -> <<"JNE is the compiler's go-to for 'if x != y'  - often used for "
                 "protocol checks (ethertype, IP protocol, TCP flags).">>;
        jle -> <<"JLE is commonly used for bounds checks: "
                 "'if data + N <= data_end' ensures packet access is safe.">>;
        jgt -> <<"JGT often implements threshold checks: "
                 "'if count > limit then DROP'.">>;
        jeq -> <<"JEQ tests exact equality  - used for matching specific "
...
explain_helper(Id) ->
    {iolist_to_binary(helper_name(Id)),
     fmt("BPF helper function #~B.", [Id])}.
