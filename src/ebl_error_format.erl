%% @doc Human-readable error formatting for EBL compiler errors.
%%
...
format({{expected, Exp, got, Got}, {L, C}}) ->
    io_lib:format("Line ~B, Col ~B: expected ~s, got ~s",
                  [L, C, token_name(Exp), token_name(Got)]);
format({{expected_ident, Got}, {L, C}}) ->
    io_lib:format("Line ~B, Col ~B: expected identifier, got ~s",
                  [L, C, token_name(Got)]);
format({{expected_type_ident, Got}, {L, C}}) ->
    io_lib:format("Line ~B, Col ~B: expected type name, got ~s",
                  [L, C, token_name(Got)]);
format({{expected_type, Got}, {L, C}}) ->
    io_lib:format("Line ~B, Col ~B: expected type expression, got ~s",
                  [L, C, token_name(Got)]);
format({{expected_integer, Got}, {L, C}}) ->
    io_lib:format("Line ~B, Col ~B: expected integer, got ~s",
                  [L, C, token_name(Got)]);
format({{expected_atom, Got}, {L, C}}) ->
    io_lib:format("Line ~B, Col ~B: expected atom literal, got ~s",
                  [L, C, token_name(Got)]);
format({{expected_prog_type, Got}, {L, C}}) ->
    io_lib:format("Line ~B, Col ~B: expected program type (xdp), got ~s",
                  [L, C, token_name(Got)]);
format({{expected_map_kind, Got}, {L, C}}) ->
    io_lib:format("Line ~B, Col ~B: expected map kind (hash, array, ...), got ~s",
                  [L, C, token_name(Got)]);
format({{expected_ident, Expected, got, Got}, {L, C}}) ->
    io_lib:format("Line ~B, Col ~B: expected '~s', got ~s",
                  [L, C, Expected, token_name(Got)]);
format({{unexpected_in_program, Got}, {L, C}}) ->
    io_lib:format("Line ~B, Col ~B: unexpected ~s in program body "
                  "(expected fn, map, type, const, or end)",
                  [L, C, token_name(Got)]);
format({{unexpected_token, Got}, {L, C}}) ->
    io_lib:format("Line ~B, Col ~B: unexpected ~s",
                  [L, C, token_name(Got)]);
format({{unexpected_pattern, Got}, {L, C}}) ->
    io_lib:format("Line ~B, Col ~B: unexpected ~s in pattern",
                  [L, C, token_name(Got)]);
format({expected_range_op, {L, C}}) ->
    io_lib:format("Line ~B, Col ~B: expected range operator (.. or ..=)",
                  [L, C]);
format({expected_direction, {L, C}}) ->
    io_lib:format("Line ~B, Col ~B: expected direction (:ingress or :egress)",
                  [L, C]);
...
format({unknown_field, TypeName, Field, {L, C}}) ->
    io_lib:format("Line ~B, Col ~B: type '~s' has no field '~s'",
                  [L, C, TypeName, Field]);
...
token_name(eof) -> <<"end of input">>;
token_name(newline) -> <<"newline">>;
token_name(lparen) -> <<"'('">>;
token_name(rparen) -> <<"')'">>;
token_name(lbrace) -> <<"'{'">>;
token_name(rbrace) -> <<"'}'">>;
...
token_name(arrow) -> <<"'->'">>;
token_name(do_kw) -> <<"'do'">>;
token_name(end_kw) -> <<"'end'">>;
token_name(fn_kw) -> <<"'fn'">>;
token_name(let_kw) -> <<"'let'">>;
token_name(if_kw) -> <<"'if'">>;
token_name(else_kw) -> <<"'else'">>;
token_name(elif_kw) -> <<"'elif'">>;
token_name(for_kw) -> <<"'for'">>;
token_name(in_kw) -> <<"'in'">>;
token_name(return_kw) -> <<"'return'">>;
token_name(match_kw) -> <<"'match'">>;
token_name(map_kw) -> <<"'map'">>;
token_name(type_kw) -> <<"'type'">>;
token_name(const_kw) -> <<"'const'">>;
token_name(xdp_kw) -> <<"'xdp'">>;
token_name(tc_kw) -> <<"'tc'">>;
token_name(cgroup_kw) -> <<"'cgroup'">>;
token_name(socket_kw) -> <<"'socket'">>;
token_name(ident) -> <<"identifier">>;
token_name(type_ident) -> <<"type name">>;
token_name(integer_lit) -> <<"integer">>;
token_name(atom_lit) -> <<"atom">>;
token_name(true_kw) -> <<"'true'">>;
token_name(false_kw) -> <<"'false'">>;
