...
map_lookup_with_null_check_test() ->
    ...
    Bin = iolist_to_binary([
        %% 0: dummy (not real map_lookup setup)
        ebpf_insn:mov64_imm(0, 0),
        %% 1: r1 = map ptr (setup)
        ebpf_insn:mov64_imm(1, 0),
        %% 2: r2 = key ptr (setup)
        ebpf_insn:mov64_imm(2, 0),
        %% 3: call map_lookup_elem
        ebpf_insn:call(1),
        %% 4: if r0 == 0 goto +2 (skip load, go to exit)
        ebpf_insn:jeq_imm(0, 0, 2),
        %% 5: r1 = [r0+0] (safe, null checked)
        ebpf_insn:ldxdw(1, 0, 0),
        %% 6: r0 = 0
        ebpf_insn:mov64_imm(0, 0),
        %% 7: exit
        ebpf_insn:exit_insn()
...
uninitialized_register_test() ->
    ...
    ?assert(
        lists:any(
            fun
                ({uninitialized_register, 5, _}) -> true;
                (_) -> false
            end,
            Errors
        )
...
instruction_limit_test() ->
    ...
    ?assert(
        lists:any(
            fun
                ({instruction_limit_exceeded, _, _}) -> true;
                (_) -> false
            end,
            Errors
        )
...
backwards_jump_test() ->
    ...
    Bin = iolist_to_binary([
        ...
        ebpf_insn:mov64_imm(0, 0),
        %% PC 1: loop counter
        ebpf_insn:mov64_imm(2, 5),
        %% PC 2: r0 += 1
        ebpf_insn:add64_imm(0, 1),
        %% PC 3: r2 -= 1
        ebpf_insn:sub64_imm(2, 1),
        %% PC 4: if r2 > 0 goto PC 2 (off = -3)
        ebpf_insn:jgt_imm(2, 0, -3),
        %% PC 5
        ebpf_insn:exit_insn()
    ]),
    ?assertEqual(ok, ebl_pre_verify:check(Bin)).
...
write_r10_test() ->
    ...
    Bin = iolist_to_binary([
        %% Write to R10 (illegal in real BPF)
...
positive_stack_offset_test() ->
    ...
    Bin = iolist_to_binary([
        %% [r10+0] — offset 0 is invalid
        ebpf_insn:stxdw(10, 0, 1),
        ebpf_insn:mov64_imm(0, 0),
        ebpf_insn:exit_insn()
    ]),
    {error, Errors} = ebl_pre_verify:check(Bin),
...
jump_past_end_test() ->
    ...
    Bin = iolist_to_binary([
        ...
        %% target way out of bounds
        ebpf_insn:jeq_imm(0, 1, 50),
        ebpf_insn:exit_insn()
    ]),
    {error, Errors} = ebl_pre_verify:check(Bin),
    ?assert(
...
exit_without_r0_init_test() ->
    ...
    ?assert(
        lists:any(
            fun
                ({uninitialized_register, 0, _}) -> true;
                (_) -> false
            end,
            Errors
        )
...
call_clobbers_r1_r5_test() ->
    ...
    Bin = iolist_to_binary([
        ...
        %% init R2
        ebpf_insn:mov64_imm(2, 0),
        %% call ktime_get_ns; clobbers R1-R5
        ebpf_insn:call(5),
        %% read R1 — not initialized after call!
    ...
    ?assert(
        lists:any(
            fun
                ({uninitialized_register, 1, _}) -> true;
                (_) -> false
            end,
            Errors
        )
...
all_reachable_conditional_test() ->
    ...
    Bin = iolist_to_binary([
        %% PC 0
        ebpf_insn:mov64_imm(0, 1),
        %% PC 1: if r0==0 goto PC 3
        ebpf_insn:jeq_imm(0, 0, 1),
        %% PC 2: reached via fall-through
        ebpf_insn:mov64_imm(0, 2),
        %% PC 3: reached via both paths
        ebpf_insn:exit_insn()
    ]),
    Result = ebl_pre_verify:check(Bin),
    case Result of
...
linear_no_unreachable_test() ->
    ...
    Bin = iolist_to_binary([
        ebpf_insn:mov64_imm(0, 0),
        ebpf_insn:exit_insn(),
        %% Dead code but no jump in program
        ebpf_insn:mov64_imm(0, 1),
        ebpf_insn:exit_insn()
    ]),
    Result = ebl_pre_verify:check(Bin),
...
helper_arg_map_lookup_scalar_r1_test() ->
    ...
    Bin = iolist_to_binary([
        ebpf_insn:mov64_imm(0, 0),
        %% R1 = scalar (should be ptr_to_map)
        ebpf_insn:mov64_imm(1, 42),
        %% R2 = scalar (should be ptr_to_stack)
        ebpf_insn:mov64_imm(2, 0),
        %% map_lookup_elem
        ebpf_insn:call(1),
        ebpf_insn:mov64_imm(0, 0),
    ...
    ?assert(
        lists:any(
            fun
                ({invalid_helper_arg, 1, 1, ptr_to_map, _, _}) -> true;
                (_) -> false
            end,
            Errors
        )
...
helper_non_map_no_typecheck_test() ->
    ...
    Bin = iolist_to_binary([
        ebpf_insn:mov64_imm(0, 0),
        %% ktime_get_ns — no arg type requirements
        ebpf_insn:call(5),
        ebpf_insn:exit_insn()
    ]),
    Result = ebl_pre_verify:check(Bin),
    case Result of
        ...
        {error, Errors} ->
            ?assertNot(
                lists:any(
                    fun
                        ({invalid_helper_arg, _, _, _, _, _}) -> true;
                        (_) -> false
                    end,
                    Errors
                )
...
compiler_output_acceptance_test_() ->
    Programs = [
        ...
        {"drop", <<"xdp test do\n  fn main(ctx) -> action do\n    return :drop\n  end\nend">>},
        {"literal_42", <<"xdp test do\n  fn main(ctx) -> u64 do\n    return 42\n  end\nend">>},
        {"literal_0", <<"xdp test do\n  fn main(ctx) -> u64 do\n    return 0\n  end\nend">>},
