...
p1_xdp_drop_test() ->
    ...
    Src = <<"xdp test do\n"
            ...
            "  end\n"
            "end">>,
    run_pipeline(Src, #{packet => Pkt}, 1).  %% XDP_DROP = 1
...
p1_xdp_pass_test() ->
    ...
    Src = <<"xdp test do\n"
            ...
            "  end\n"
            "end">>,
    run_pipeline(Src, #{packet => Pkt}, 2).  %% XDP_PASS = 2
...
p8_helper_ktime_test() ->
    ...
    Bytecode = ebpf_insn:assemble([
        ebpf_insn:call(5),              %% R0 = ktime_get_ns()
        ebpf_insn:jgt_imm(0, 0, 1),     %% if R0 > 0 goto +1
        ebpf_insn:mov64_imm(0, 1),       %% R0 = 1 (DROP)
        ebpf_insn:mov64_imm(0, 2),       %% R0 = 2 (PASS) — jump target
        ebpf_insn:exit_insn()
    ]),
    %% Pre-verify
    ?assertEqual(ok, ebl_pre_verify:check(Bytecode)),
    ...
    ?assertEqual(2, Result).  %% ktime > 0 always
