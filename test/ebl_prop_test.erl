...
ip_addr() ->
    {proper_types:byte(), proper_types:byte(),
     proper_types:byte(), proper_types:byte()}.
...
tcp_pkt() ->
    ?PROP_LET({SrcIP, DstIP, SrcPort, DstPort, Flags, PayloadSize},
         {ip_addr(), ip_addr(),
          proper_types:range(1, 65535), proper_types:range(1, 65535),
          tcp_flags(), proper_types:range(0, 200)},
...
udp_pkt() ->
    ?PROP_LET({SrcIP, DstIP, SrcPort, DstPort, PayloadSize},
         {ip_addr(), ip_addr(),
          proper_types:range(1, 65535), proper_types:range(1, 65535),
          proper_types:range(0, 200)},
         ebpf_test_pkt:udp(#{src_ip => SrcIP, dst_ip => DstIP,
                              src_port => SrcPort, dst_port => DstPort,
                              payload => <<0:(PayloadSize*8)>>})).

icmp_pkt() ->
    ?PROP_LET({SrcIP, DstIP},
         {ip_addr(), ip_addr()},
         ebpf_test_pkt:icmp(#{src_ip => SrcIP, dst_ip => DstIP})).

arp_pkt() ->
    ?PROP_LET({SenderIP, TargetIP},
         {ip_addr(), ip_addr()},
...
non_ipv4_pkt() ->
    proper_types:oneof([
        arp_pkt(),
        ?PROP_LET(Payload, proper_types:binary(20),
...
short_pkt() ->
    ?PROP_LET(Size, proper_types:range(1, 33),
         <<0:(Size*8)>>).
...
prop_non_ipv4_always_pass(Path, MapSpecs) ->
    ?FORALL(Pkt, non_ipv4_pkt(),
            run_xdp(Path, Pkt, MapSpecs) =:= ?XDP_PASS).

prop_15_non_ipv4_test_() ->
    prop_test(fun() -> prop_non_ipv4_always_pass(
        "examples/15_syn_flood_protect.ebl", [{hash, 4, 8, 65536}]) end).

prop_16_non_ipv4_test_() ->
    prop_test(fun() -> prop_non_ipv4_always_pass(
        "examples/16_port_firewall.ebl", [{hash, 4, 8, 1024}]) end).

prop_17_non_ipv4_test_() ->
    prop_test(fun() -> prop_non_ipv4_always_pass(
        "examples/17_ttl_filter.ebl", [{hash, 4, 8, 16384}]) end).

prop_18_non_ipv4_test_() ->
    prop_test(fun() -> prop_non_ipv4_always_pass(
        "examples/18_icmp_rate_limiter.ebl", [{hash, 4, 8, 32768}]) end).

prop_19_non_ipv4_test_() ->
    prop_test(fun() -> prop_non_ipv4_always_pass(
        "examples/19_dns_amplification.ebl", [{hash, 4, 8, 32768}]) end).

prop_20_non_ipv4_test_() ->
    prop_test(fun() -> prop_non_ipv4_always_pass(
        "examples/20_subnet_firewall.ebl", [{hash, 4, 8, 4096}]) end).

prop_21_non_ipv4_test_() ->
    prop_test(fun() -> prop_non_ipv4_always_pass(
        "examples/21_port_scan_detect.ebl", [{hash, 4, 8, 16384}]) end).

prop_22_non_ipv4_test_() ->
    prop_test(fun() -> prop_non_ipv4_always_pass(
        "examples/22_bandwidth_monitor.ebl", [{hash, 4, 8, 65536}]) end).

%%% --- Short packets always PASS ---

prop_short_always_pass(Path, MapSpecs) ->
    ?FORALL(Pkt, short_pkt(),
            run_xdp(Path, Pkt, MapSpecs) =:= ?XDP_PASS).

prop_15_short_test_() ->
    prop_test(fun() -> prop_short_always_pass(
        "examples/15_syn_flood_protect.ebl", [{hash, 4, 8, 65536}]) end).
...
prop_15_valid_action_test_() ->
    prop_test(fun() -> prop_always_valid_action(
        "examples/15_syn_flood_protect.ebl", [{hash, 4, 8, 65536}]) end).
...
prop_15_non_tcp_pass_test_() ->
    prop_test(fun() ->
        ?FORALL(Pkt, proper_types:oneof([udp_pkt(), icmp_pkt()]),
                run_xdp("examples/15_syn_flood_protect.ebl", Pkt,
                         [{hash, 4, 8, 65536}]) =:= ?XDP_PASS) end).

prop_15_tcp_no_syn_pass_test_() ->
    prop_test(fun() ->
        ?FORALL({SrcIP, DstIP, DstPort},
                {ip_addr(), ip_addr(), proper_types:range(1, 65535)},
                run_xdp("examples/15_syn_flood_protect.ebl",
                         ebpf_test_pkt:tcp(#{src_ip => SrcIP, dst_ip => DstIP,
                                              dst_port => DstPort, flags => [ack]}),
                         [{hash, 4, 8, 65536}]) =:= ?XDP_PASS) end).

prop_16_icmp_pass_test_() ->
    prop_test(fun() ->
        ?FORALL(Pkt, icmp_pkt(),
                run_xdp("examples/16_port_firewall.ebl", Pkt,
                         [{hash, 4, 8, 1024}]) =:= ?XDP_PASS) end).

prop_21_non_rst_pass_test_() ->
    prop_test(fun() ->
        ?FORALL({SrcIP, DstIP, DstPort, Flags},
                {ip_addr(), ip_addr(), proper_types:range(1, 65535),
                 proper_types:oneof([[syn], [ack], [syn, ack], [fin], [fin, ack], [psh, ack]])},
                run_xdp("examples/21_port_scan_detect.ebl",
                         ebpf_test_pkt:tcp(#{src_ip => SrcIP, dst_ip => DstIP,
                                              dst_port => DstPort, flags => Flags}),
                         [{hash, 4, 8, 16384}]) =:= ?XDP_PASS) end).

prop_18_non_icmp_pass_test_() ->
    prop_test(fun() ->
        ?FORALL(Pkt, proper_types:oneof([tcp_pkt(), udp_pkt()]),
                run_xdp("examples/18_icmp_rate_limiter.ebl", Pkt,
                         [{hash, 4, 8, 32768}]) =:= ?XDP_PASS) end).

prop_19_non_dns_pass_test_() ->
    prop_test(fun() ->
        ?FORALL(Pkt, proper_types:oneof([tcp_pkt(), icmp_pkt()]),
                run_xdp("examples/19_dns_amplification.ebl", Pkt,
                         [{hash, 4, 8, 32768}]) =:= ?XDP_PASS) end).

prop_19_udp_non53_pass_test_() ->
    prop_test(fun() ->
        ?FORALL({SrcIP, DstIP, SrcPort},
                {ip_addr(), ip_addr(), proper_types:range(54, 65535)},
                run_xdp("examples/19_dns_amplification.ebl",
                         ebpf_test_pkt:udp(#{src_ip => SrcIP, dst_ip => DstIP,
                                              src_port => SrcPort}),
                         [{hash, 4, 8, 32768}]) =:= ?XDP_PASS) end).
...
prop_17_normal_ttl_pass_test_() ->
    prop_test(fun() ->
        ?FORALL({SrcIP, DstIP},
                {ip_addr(), ip_addr()},
                run_xdp("examples/17_ttl_filter.ebl",
                         ebpf_test_pkt:tcp(#{src_ip => SrcIP, dst_ip => DstIP}),
                         [{hash, 4, 8, 16384}]) =:= ?XDP_PASS) end).
        ...
        ?assert(proper:quickcheck(
            ?FORALL({IP_A, IP_B},
                ...
                begin
                    ...
                    MapState = ebpf_vm:create_maps([{hash, 4, 8, 65536}]),
                    try
                        PktA = ebpf_test_pkt:tcp(#{src_ip => IP_A, dst_ip => {10,0,0,2},
                                                    flags => [syn]}),
                        PktB = ebpf_test_pkt:tcp(#{src_ip => IP_B, dst_ip => {10,0,0,2},
                                                    flags => [syn]}),
...
stateful_19_dns_amp_threshold_test_() ->
    {timeout, 60, fun() ->
        ...
        try
            BigPayload = <<0:(600*8)>>,
            Pkt = ebpf_test_pkt:udp(#{src_ip => {8,8,8,8}, dst_ip => {10,0,0,1},
                                       src_port => 53, dst_port => 1024,
                                       payload => BigPayload}),
...
stateful_22_bwmon_accumulation_test_() ->
    {timeout, 60, fun() ->
        ...
        try
            ...
            SrcIPKey = (10 bsl 24) bor 1,  %% big-endian u32
...
stateful_22_bwmon_random_test_() ->
    {timeout, 60, fun() ->
        ?assert(proper:quickcheck(
            ?FORALL(PayloadSize, proper_types:range(0, 500),
                begin
                    {ok, Bin} = ebl_compile:file("examples/22_bandwidth_monitor.ebl"),
                    MapState = ebpf_vm:create_maps([{hash, 4, 8, 65536}]),
                    try
        ...
        ?assert(proper:quickcheck(
            ?FORALL({SrcIP, DstIP},
                ...
                begin
                    ...
                    MapState = ebpf_vm:create_maps([{hash, 4, 8, 16384}]),
                    try
                        Pkt = ebpf_test_pkt:tcp(#{src_ip => SrcIP, dst_ip => DstIP,
                                                    flags => [rst]}),
                        {Results, _} = run_multi(Bin, Pkt, MapState, 35),
    ...
    {timeout, 60, fun() ->
        ...
        try
            ...
            Pkts = lists:flatten([
                ...
                lists:duplicate(10, SynPkt),
                lists:duplicate(10, SynPkt)  %% total SYN = 50
            ]),
            {Results, _} = run_sequence(Bin, Pkts, MapState),
...
prop_xval_15_test_() ->
    {timeout, 120, fun() ->
        ?assert(proper:quickcheck(prop_xval(
