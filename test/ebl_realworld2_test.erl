...
dns_amp_pass_small_dns_response_test() ->
    ...
    Pkt = ebpf_test_pkt:udp(#{src_ip => {8, 8, 8, 8},
                               ...
                               dst_port => 1024,
                               payload => <<0:64>>}),
    assert_ebl_xdp("examples/19_dns_amplification.ebl", Pkt,
                     [{hash, 4, 8, 32768}], 2).
...
dns_amp_pass_tcp_test() ->
    ...
    Pkt = ebpf_test_pkt:tcp(#{src_ip => {8, 8, 8, 8},
                               dst_ip => {10, 0, 0, 1}}),
    assert_ebl_xdp("examples/19_dns_amplification.ebl", Pkt,
                     [{hash, 4, 8, 32768}], 2).
...
subnet_fw_pass_unblocked_ip_test() ->
    ...
    Pkt = ebpf_test_pkt:tcp(#{src_ip => {10, 0, 0, 1},
                               dst_ip => {10, 0, 0, 2}}),
    assert_ebl_xdp("examples/20_subnet_firewall.ebl", Pkt,
                     [{hash, 4, 8, 4096}], 2).
...
subnet_fw_pass_udp_test() ->
    Pkt = ebpf_test_pkt:udp(#{src_ip => {192, 168, 1, 100},
                               dst_ip => {10, 0, 0, 1}}),
    assert_ebl_xdp("examples/20_subnet_firewall.ebl", Pkt,
                     [{hash, 4, 8, 4096}], 2).

subnet_fw_pass_icmp_test() ->
    Pkt = ebpf_test_pkt:icmp(#{src_ip => {172, 16, 0, 5},
                                dst_ip => {10, 0, 0, 1}}),
    assert_ebl_xdp("examples/20_subnet_firewall.ebl", Pkt,
                     [{hash, 4, 8, 4096}], 2).
...
portscan_pass_syn_test() ->
    ...
    Pkt = ebpf_test_pkt:tcp(#{src_ip => {10, 0, 0, 1},
                               dst_ip => {10, 0, 0, 2},
                               flags => [syn]}),
    assert_ebl_xdp("examples/21_port_scan_detect.ebl", Pkt,
                     [{hash, 4, 8, 16384}], 2).
...
bwmon_pass_tcp_test() ->
    ...
    Pkt = ebpf_test_pkt:tcp(#{src_ip => {10, 0, 0, 1},
                               dst_ip => {10, 0, 0, 2}}),
    assert_ebl_xdp("examples/22_bandwidth_monitor.ebl", Pkt,
                     [{hash, 4, 8, 65536}], 2).
...
xval_dns_amp_small_test() ->
    ...
    assert_ebl_xdp_xval("examples/19_dns_amplification.ebl", Pkt,
                          [{hash, 4, 8, 32768}], 2).

xval_subnet_fw_test() ->
    Pkt = ebpf_test_pkt:tcp(#{src_ip => {10, 0, 0, 1},
                               dst_ip => {10, 0, 0, 2}}),
    assert_ebl_xdp_xval("examples/20_subnet_firewall.ebl", Pkt,
                          [{hash, 4, 8, 4096}], 2).
...
xval_portscan_syn_test() ->
    Pkt = ebpf_test_pkt:tcp(#{src_ip => {10, 0, 0, 1},
                               dst_ip => {10, 0, 0, 2},
                               flags => [syn]}),
    assert_ebl_xdp_xval("examples/21_port_scan_detect.ebl", Pkt,
                          [{hash, 4, 8, 16384}], 2).
...
xval_bwmon_tcp_test() ->
    Pkt = ebpf_test_pkt:tcp(#{src_ip => {10, 0, 0, 1},
                               dst_ip => {10, 0, 0, 2}}),
    assert_ebl_xdp_xval("examples/22_bandwidth_monitor.ebl", Pkt,
                          [{hash, 4, 8, 65536}], 2).
