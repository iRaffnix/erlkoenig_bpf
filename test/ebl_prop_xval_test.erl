...
ip_addr() ->
    {proper_types:range(1, 254), proper_types:range(0, 255),
     proper_types:range(0, 255), proper_types:range(1, 254)}.
...
tcp_pkt() ->
    ?PROP_LET({SrcIP, DstIP, SrcPort, DstPort, Flags, PayloadSize, TTL},
         {ip_addr(), ip_addr(),
...
udp_pkt() ->
    ?PROP_LET({SrcIP, DstIP, SrcPort, DstPort, PayloadSize, TTL},
         {ip_addr(), ip_addr(),
          proper_types:range(1, 65535), proper_types:range(1, 65535),
          proper_types:range(0, 300), proper_types:range(1, 255)},
...
xval_single(Bin, Pkt, MapSpecs) ->
    ...
    case ebpf_ubpf:start() of
        {ok, Port} ->
            try
                lists:foreach(fun({_Type, KS, VS, Max}) ->
                ...
                case ebpf_ubpf:load(Port, Bin) of
                    ...
                    {error, _} ->
                        true  %% uBPF doesn't support this program
                end
            after
                ebpf_ubpf:stop(Port)
            end;
...
prop_xval_15_any_test_() ->
    prop_test(fun() -> prop_xval(
        "examples/15_syn_flood_protect.ebl", [{hash, 4, 8, 65536}]) end, 200).

prop_xval_15_short_test_() ->
    prop_test(fun() -> prop_xval_short(
        "examples/15_syn_flood_protect.ebl", [{hash, 4, 8, 65536}]) end, 100).

%%% --- 16: Port Firewall ---

prop_xval_16_any_test_() ->
    prop_test(fun() -> prop_xval(
        "examples/16_port_firewall.ebl", [{hash, 4, 8, 1024}]) end, 200).

%%% --- 17: TTL Filter ---

prop_xval_17_any_test_() ->
    prop_test(fun() -> prop_xval(
        "examples/17_ttl_filter.ebl", [{hash, 4, 8, 16384}]) end, 200).

%%% --- 18: ICMP Rate Limiter ---

prop_xval_18_any_test_() ->
    prop_test(fun() -> prop_xval(
        "examples/18_icmp_rate_limiter.ebl", [{hash, 4, 8, 32768}]) end, 200).

%%% --- 19: DNS Amplification ---

prop_xval_19_any_test_() ->
    prop_test(fun() -> prop_xval(
        "examples/19_dns_amplification.ebl", [{hash, 4, 8, 32768}]) end, 200).

%%% --- 20: Subnet Firewall ---

prop_xval_20_any_test_() ->
    prop_test(fun() -> prop_xval(
        "examples/20_subnet_firewall.ebl", [{hash, 4, 8, 4096}]) end, 200).

%%% --- 21: Port Scan Detect ---

prop_xval_21_any_test_() ->
    prop_test(fun() -> prop_xval(
        "examples/21_port_scan_detect.ebl", [{hash, 4, 8, 16384}]) end, 200).

%%% --- 22: Bandwidth Monitor ---

prop_xval_22_any_test_() ->
    prop_test(fun() -> prop_xval(
        "examples/22_bandwidth_monitor.ebl", [{hash, 4, 8, 65536}]) end, 200).
